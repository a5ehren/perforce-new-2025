...
import { P4Options, P4DescribeResult, P4Annotation, P4JobSummary, P4ChangeSummary, P4CommandContext, P4Info } from './p4/p4Types';
import { getP4OptionsFromConfig } from './p4/p4Utils';
import { RepositoryStateManager, P4File } from './RepositoryStateManager';
import { p4edit, p4revert, p4add, p4delete, p4sync, p4move, p4diff2, p4resolve, p4where } from './p4/fileCommands';
import { p4newChangeSpec, p4editChangeSpec, p4saveChangeSpec, p4submit, p4describe, p4changes } from './p4/changelistCommands';
...
export class PerforceSCMProvider implements vscode.Disposable {
    ...
    constructor(contextUri: vscode.Uri, outputChannel: vscode.OutputChannel) {
        ...
        this._repositoryStateManager = new RepositoryStateManager(this._perforceService, this._outputChannel, p4Options);

        this._scm.inputBox.placeholder = 'Enter changelist description';

        this._repositoryStateManager.onDidChange(this.onStateChange, this, this._disposables);
        ...
        this._outputChannel.appendLine(`Perforce SCM Provider initialized for ${scmTitle}`);
    ...
    private onStateChange(): void {
        this._outputChannel.appendLine(`Updating SCM view for ${this._scm.label}...`);
        ...
        changelists.forEach(change => {
            ...
            group.resourceStates = filesInChange.map(file => this.createResourceState(file));
            
            groups.push(group);
        });

        const config = vscode.workspace.getConfiguration('perforce', this._scm.rootUri);
    ...
    private createResourceState(file: P4File): vscode.SourceControlResourceState {
        ...
        if (file.changelist) {tooltip += `\nChangelist: ${file.changelist}`;}
        if (file.type) {tooltip += `\nType: ${file.type}`;}

        const strikeThrough = file.action === 'delete' || file.action === 'move/delete' || file.action === 'purge' || file.action === 'archive';
        ...
        if (file.diffStatus === 'unresolved' || file.diffStatus === 'reresolvable') {
    ...
    public async editFile(resourceUri: vscode.Uri): Promise<void> {
        this._outputChannel.appendLine(`SCM Provider: Request to edit ${resourceUri.fsPath}`);
        ...
        try {
            ...
            await this.refresh(); 
             ...
             this._outputChannel.appendLine(`SCM Provider: Error during p4edit for ${resourceUri.fsPath}: ${error.message}`);
            throw error;
        }
    }

    public async revertFile(resourceUri: vscode.Uri): Promise<void> {
         this._outputChannel.appendLine(`SCM Provider: Request to revert ${resourceUri.fsPath}`);
        const p4Options = getP4OptionsFromConfig(resourceUri);
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            await this.refresh();
             ...
             this._outputChannel.appendLine(`SCM Provider: Error during p4revert for ${resourceUri.fsPath}: ${error.message}`);
            throw error;
        }
    }

    public async addFile(resourceUri: vscode.Uri): Promise<void> {
         this._outputChannel.appendLine(`SCM Provider: Request to add ${resourceUri.fsPath}`);
        const p4Options = getP4OptionsFromConfig(resourceUri);
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            await this.refresh();
             ...
             this._outputChannel.appendLine(`SCM Provider: Error during p4add for ${resourceUri.fsPath}: ${error.message}`);
            throw error;
        }
    }

    public async deleteFile(resourceUri: vscode.Uri): Promise<void> {
         this._outputChannel.appendLine(`SCM Provider: Request to delete ${resourceUri.fsPath}`);
        const p4Options = getP4OptionsFromConfig(resourceUri);
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            await this.refresh();
             ...
             this._outputChannel.appendLine(`SCM Provider: Error during p4delete for ${resourceUri.fsPath}: ${error.message}`);
            throw error;
        }
    }
    
    public async syncFiles(resourceUris: vscode.Uri[]): Promise<void> {
        this._outputChannel.appendLine(`SCM Provider: Request to sync ${resourceUris.length > 0 ? resourceUris.length + ' specific files' : 'all files' } in ${this.rootUri.fsPath}`);
        ...
        try {
            ...
            await this.refresh();
             ...
             this._outputChannel.appendLine(`SCM Provider: Error during p4sync for ${this.rootUri.fsPath}: ${error.message}`);
            throw error;
        }
    }
    
    public async moveFile(sourceUri: vscode.Uri, targetUri: vscode.Uri): Promise<void> {
         this._outputChannel.appendLine(`SCM Provider: Request to move ${sourceUri.fsPath} to ${targetUri.fsPath}`);
        const p4Options = getP4OptionsFromConfig(this.rootUri); 
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            await this.refresh();
             ...
             this._outputChannel.appendLine(`SCM Provider: Error during p4move from ${sourceUri.fsPath}: ${error.message}`);
            throw error;
        }
    }
    
    public async openResource(resourceUri: vscode.Uri, commandArgs?: any[]): Promise<void> {
        this._outputChannel.appendLine(`SCM Provider: Request to open resource ${resourceUri.fsPath}`);
        this._outputChannel.appendLine(`  Command args: ${JSON.stringify(commandArgs)}`);
        ...
        try {
            ...
            if (!depotPath) {
                this._outputChannel.appendLine(` Depot path not cached for ${localPath}, running p4 where...`);
                depotPath = await p4where(context, localPath, p4Options); 
            }
            
            if (!depotPath) {
                 this._outputChannel.appendLine(` Could not determine depot path for ${localPath}. Opening local file only.`);
            ...
            if (isShelved) {
            ...
            } else if (effectiveStatus === 'add' || effectiveStatus === 'branch' || effectiveStatus === 'import') {
                // No previous revision to diff against for add/branch/import
                 this._outputChannel.appendLine(` Opening local file for action '${effectiveStatus}'. No depot diff available.`);
                 await vscode.commands.executeCommand('vscode.open', resourceUri);
                 return;
            } else if (effectiveStatus === 'edit' || effectiveStatus === 'integrate') {
                // Diff #have vs local file
                leftSpec = `${depotPath}#have`;
                title += ` (Depot #have vs Local)`;
            } else if (effectiveStatus === 'delete' || effectiveStatus === 'move/delete') {
                ...
                title += ` (Showing deleted revision #have)`;
                 ...
                 this._outputChannel.appendLine(` TODO: Need FileSystemProvider to show depot content for ${leftSpec}`);
                 vscode.window.showInformationMessage(`Showing deleted file content requires depot FileSystemProvider (TODO). Opening local path (may not exist).`);
            ...
            if (leftSpec) {
                 ...
                 this._outputChannel.appendLine(` TODO: Need FileSystemProvider to create URI for depot content: ${leftSpec}`);
                 vscode.window.showInformationMessage(`Diffing requires depot FileSystemProvider (TODO). Opening local file ${localPath} instead.`);
                 await vscode.commands.executeCommand('vscode.open', resourceUri);
            ...
            this._outputChannel.appendLine(`SCM Provider: Error opening resource ${resourceUri.fsPath}: ${error.message}`);
    ...
    public async getNewChangeSpec(): Promise<string> {
         ...
         try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error getting new change spec: ${error.message}`);
            throw error;
        }
    }
    
    public async getChangeSpec(changelistId: string): Promise<string> {
         this._outputChannel.appendLine(`SCM Provider: Requesting change spec for ${changelistId}.`);
         const p4Options = getP4OptionsFromConfig(this.rootUri); 
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
         ...
         try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error getting change spec for ${changelistId}: ${error.message}`);
            throw error;
        }
    }
    
    public async saveChangeSpec(spec: string): Promise<string> {
         this._outputChannel.appendLine(`SCM Provider: Requesting save change spec.`);
         const p4Options = getP4OptionsFromConfig(this.rootUri); 
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
         ...
         try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error saving change spec: ${error.message}`);
            throw error;
        }
    }
    
    public async submitChange(changelistId?: string, description?: string): Promise<{ submittedChange?: string; success?: boolean; message?: string } | null> {
        const target = changelistId ?? 'default';
        this._outputChannel.appendLine(`SCM Provider: Requesting submit for changelist ${target}.`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
        const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            if (!changelistId && description) {
                 this._outputChannel.appendLine(` Warning: Submitting default changelist with description is not fully implemented. Description ignored.`);
            }
            ...
            return result;
             ...
             this._outputChannel.appendLine(`SCM Provider: Error submitting change ${target}: ${error.message}`);
            await this.refresh(); 
            throw error;
        }
    }
    
    public async describeChange(changelistId: string): Promise<P4DescribeResult | null> {
        this._outputChannel.appendLine(`SCM Provider: Requesting describe for changelist ${changelistId}.`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
        const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error describing change ${changelistId}: ${error.message}`);
            throw error;
        }
    }
    
    public async shelveChange(changelistId: string): Promise<void> {
         this._outputChannel.appendLine(`SCM Provider: Requesting shelve for changelist ${changelistId}.`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
        const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error shelving change ${changelistId}: ${error.message}`);
             await this.refresh(); 
            throw error;
        }
    }
    
    public async unshelveChange(shelvedChangeId: string, targetChangeId?: string): Promise<void> {
         this._outputChannel.appendLine(`SCM Provider: Requesting unshelve from ${shelvedChangeId} into ${targetChangeId ?? 'default'}.`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
        const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error unshelving change ${shelvedChangeId}: ${error.message}`);
             await this.refresh(); 
            throw error;
        }
    }
    ...
    public async fixJob(changelistId: string, jobId: string): Promise<void> {
        this._outputChannel.appendLine(`SCM Provider: Requesting fix job ${jobId} for change ${changelistId}.`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
        const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error fixing job ${jobId} for change ${changelistId}: ${error.message}`);
            throw error;
        }
    }
    
    public async unfixJob(changelistId: string, jobId: string): Promise<void> {
        this._outputChannel.appendLine(`SCM Provider: Requesting unfix job ${jobId} for change ${changelistId}.`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
        const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error unfixing job ${jobId} for change ${changelistId}: ${error.message}`);
            throw error;
        }
    }
    
    public async getAnnotations(resourceUri: vscode.Uri): Promise<P4Annotation[]> {
         this._outputChannel.appendLine(`SCM Provider: Requesting annotations for ${resourceUri.fsPath}.`);
        const p4Options = getP4OptionsFromConfig(resourceUri);
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            return await p4annotate(context, resourceUri.fsPath, p4Options);
             ...
             this._outputChannel.appendLine(`SCM Provider: Error getting annotations for ${resourceUri.fsPath}: ${error.message}`);
            throw error;
        }
    }

    public async resolveFile(resourceUri: vscode.Uri, resolveFlags: string[]): Promise<string> {
         this._outputChannel.appendLine(`SCM Provider: Requesting resolve for ${resourceUri.fsPath} with flags: ${resolveFlags.join(' ')}`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            const output = await p4resolve(context, resolveFlags, [resourceUri.fsPath], p4Options); 
            await this.refresh(); // State changes after resolve
            return output;
        } catch (error: any) {
             this._outputChannel.appendLine(`SCM Provider: Error during p4resolve for ${resourceUri.fsPath}: ${error.message}`);
    ...
    public async getChanges(args: string[]): Promise<P4ChangeSummary[]> {
         this._outputChannel.appendLine(`SCM Provider: Requesting changes list with args: ${args.join(' ')}`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            return await p4changes(context, p4Options, args); 
             ...
             this._outputChannel.appendLine(`SCM Provider: Error getting changes list: ${error.message}`);
            throw error;
        }
    }
    
    public async getJobs(args: string[]): Promise<P4JobSummary[]> {
         this._outputChannel.appendLine(`SCM Provider: Requesting jobs list with args: ${args.join(' ')}`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            return await p4jobs(context, p4Options, args); 
             ...
             this._outputChannel.appendLine(`SCM Provider: Error getting jobs list: ${error.message}`);
            throw error;
        }
    }
    
    public async getJobSpec(jobId: string): Promise<string> {
         this._outputChannel.appendLine(`SCM Provider: Requesting job spec for ${jobId}.`);
         const p4Options = getP4OptionsFromConfig(this.rootUri); 
         const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
         ...
         try {
            ...
            this._outputChannel.appendLine(`SCM Provider: Error getting job spec for ${jobId}: ${error.message}`);
            throw error;
        }
    }
    ...
    public async getInfo(): Promise<P4Info> {
        this._outputChannel.appendLine(`SCM Provider: Requesting p4 info for ${this.rootUri.fsPath}`);
        const p4Options = getP4OptionsFromConfig(this.rootUri);
        const context: P4CommandContext = {
            execute: this._perforceService.execute.bind(this._perforceService),
            outputChannel: this._outputChannel
        ...
        try {
            ...
            const result = await this._perforceService.execute('info', [], p4Options, false); // Explicitly set tagged=false
            ...
            if (!info.userName || !info.clientName) {
                this._outputChannel.appendLine(` Error parsing p4 info output: Missing required fields (userName, clientName). Output:\n${stdout}`);
                throw new Error('Failed to parse essential fields from p4 info output.');
            }

            this._outputChannel.appendLine(` Parsed p4 info: User=${info.userName}, Client=${info.clientName}`);
            return info as P4Info; // Cast to P4Info after validation

        } catch (error: any) {
            // Log the original error and re-throw
            this._outputChannel.appendLine(`SCM Provider: Error running/parsing p4 info: ${error.message}`);
    ...
    dispose() {
        ...
        this._repositoryStateManager.dispose();
         this._outputChannel.appendLine(`Disposing Perforce SCM Provider for ${this._scm?.label ?? '(unknown)'}`);
    }
} 