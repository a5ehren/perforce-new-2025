...
export function getP4OptionsFromConfig(resourceUri?: vscode.Uri): P4Options {
     ...
     if (client && client !== 'none') {options.P4CLIENT = client;}

     const user = config.get<string>('user');
     if (user && user !== 'none') {options.P4USER = user;}

     const port = config.get<string>('port');
     if (port && port !== 'none') {options.P4PORT = port;}

     const password = config.get<string>('password');
     if (password && password !== 'none') {options.P4PASSWD = password;}

     const charset = config.get<string>('charset');
     if (charset && charset !== 'none') {options.P4CHARSET = charset;}
