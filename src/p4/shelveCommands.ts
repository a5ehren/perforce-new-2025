 ...
 */
export async function p4shelve(context: P4CommandContext, changelist: string, options: P4Options = {}, args: string[] = []): Promise<void> {
    if (!changelist || changelist === 'default') { // 'default' might be disallowed by server for shelve
        throw new Error('A specific pending changelist number must be provided for p4 shelve.');
    }
    ...
    try {
        ...
        const result = await context.execute('shelve', effectiveArgs, options, false);
        ...
        if (result.stderr) {
            ...
            if (result.stderr.includes('no files to shelve')) {
                context.outputChannel.appendLine('Shelve: No files to shelve in the changelist.');
        ...
        if (result.stdout) {
            ...
            if (match) {
                context.outputChannel.appendLine(`Successfully shelved changelist ${match[1]}.`);
                return;
            }
        }
        ...
        context.outputChannel.appendLine('Shelve command finished, but could not confirm success from output.');
 ...
 */
export async function p4unshelve(context: P4CommandContext, shelvedChange: string, targetChange?: string, options: P4Options = {}, args: string[] = []): Promise<void> {
    if (!shelvedChange) {
        throw new Error('Shelved changelist number must be provided for p4 unshelve.');
    }
    ...
    try {
        ...
        const result = await context.execute('unshelve', effectiveArgs, options, false);
        ...
        if (result.stderr) {
            ...
            if (result.stderr.includes('must resolve')) {
                context.outputChannel.appendLine('Unshelve requires resolve. Files likely opened for integrate.');
        ...
        // Assuming if no error was thrown, and 'must resolve' wasn't the only output, it succeeded.
         context.outputChannel.appendLine(`\`${commandDesc}\` executed. Check output/status for results (resolve may be needed).`);
