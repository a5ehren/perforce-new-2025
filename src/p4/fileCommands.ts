...
import { P4Options, P4Result, P4OpenedFile, P4StatusFile, P4CommandContext, P4DescribeResult } from './p4Types';
 ...
 */
export async function p4where(context: P4CommandContext, depotOrClientPath: string, options: P4Options = {}): Promise<string | null> {
    if (!depotOrClientPath) {
        return null;
    }
    context.outputChannel.appendLine(`Executing \`p4 where ${depotOrClientPath}\`...`);
    try {
        // Execute 'p4 where' - no -G needed
        const result = await context.execute('where', [depotOrClientPath], options, false);

        if (result.stderr) {
            // 'p4 where' often reports errors like 'not in client view' to stderr
            context.outputChannel.appendLine(`\`p4 where ${depotOrClientPath}\` reported error: ${result.stderr.trim()}`);
            if (result.stderr.includes('not in client view') || result.stderr.includes('no such file(s)')) {
        ...
        if (result.stdout) {
            ...
            for (const line of lines) {
                ...
                if (parts.length >= 3) {
                    ...
                    if (depotPath === depotOrClientPath || clientPath === depotOrClientPath) {
                         context.outputChannel.appendLine(`  \`p4 where\` mapped "${depotOrClientPath}" to local path: "${localPath}"`);
                        // Basic validation: does it look like a plausible path?
                        if (localPath && localPath !== '/dev/null' && !localPath.startsWith('-')) { // Exclude common non-paths
                            return localPath;
                        }
                    }
                }
            ...
            // If loop finishes without returning, no matching line found
             context.outputChannel.appendLine(`  \`p4 where ${depotOrClientPath}\` output did not contain a matching mapping line.`);
             return null;
        }

        // No stdout and no specific stderr error? Unlikely but possible.
        context.outputChannel.appendLine(`  \`p4 where ${depotOrClientPath}\` produced no usable output.`);
        return null;

    } catch (error: any) {
         // Handle errors from execute() itself (e.g., p4 command not found)
         context.outputChannel.appendLine(`Error executing \`p4 where ${depotOrClientPath}\`: ${error.message}`);
 ...
 */
export async function p4opened(context: P4CommandContext, options: P4Options = {}): Promise<P4OpenedFile[]> {
    context.outputChannel.appendLine('Executing `p4 opened -G`...');
    try {
        const result = await context.execute('opened', [], options, true);

        if (result.stderr && !result.stderr.includes('File(s) not opened on this client')) {
            // Log unexpected stderr, but proceed if we got parsed output
            context.outputChannel.appendLine(`Warning during \`p4 opened -G\`: ${result.stderr}`);
        }
        ...
        if (Array.isArray(result.parsedOutput)) {
            ...
            const openedFiles: P4OpenedFile[] = result.parsedOutput.map((item: any) => ({
            ...
            context.outputChannel.appendLine(`\`p4 opened -G\` found ${openedFiles.length} files.`);
            return openedFiles;
        } else if (result.stderr && result.stderr.includes('File(s) not opened on this client')) {
            // This is not an error, just means no files are open
            context.outputChannel.appendLine('`p4 opened -G`: No files opened on this client.');
            return [];
        } else {
            // Handle cases where parsing might have failed or output was unexpected
            context.outputChannel.appendLine('`p4 opened -G\` did not return a valid array in parsedOutput.');
            return [];
        }
    } catch (error: any) {
        context.outputChannel.appendLine(`Error executing \`p4 opened -G\`: ${error.message}`);
 ...
 */
export async function p4status(context: P4CommandContext, options: P4Options = {}): Promise<P4StatusFile[]> {
    ...
    try {
        ...
        if (result.stderr) {
            ...
            context.outputChannel.appendLine(`Warning/Info during \`p4 status -G\`: ${result.stderr}`);
        }

        if (Array.isArray(result.parsedOutput)) {
            const statusFiles: P4StatusFile[] = result.parsedOutput.map((item: any) => ({
            ...
            context.outputChannel.appendLine(`\`p4 status -G\` reported status for ${statusFiles.length} files.`);
            return statusFiles;
        } else {
            // p4 status with -G returns an empty list if nothing matches,
            // so an empty array is a valid successful result.
             context.outputChannel.appendLine('`p4 status -G`: No file statuses reported (or output was not an array).');
            return [];
        }
    } catch (error: any) {
        context.outputChannel.appendLine(`Error executing \`p4 status -G\`: ${error.message}`);
 ...
 */
export async function p4add(context: P4CommandContext, filePath: string, options: P4Options = {}): Promise<void> {
    if (!filePath) {
    ...
    try {
        ...
        if (result.stderr) {
            context.outputChannel.appendLine(`Warning/Info during \`p4 add ${filePath}\`: ${result.stderr}`);
        ...
        context.outputChannel.appendLine(`\`p4 add ${filePath}\` executed successfully.`);

    } catch (error: any) {
        // execute() already logs the error
        context.outputChannel.appendLine(`Error executing \`p4 add ${filePath}\`: ${error.message}`);
 ...
 * @returns A promise that resolves on success, rejects on failure.
 */
export async function p4edit(context: P4CommandContext, filePath: string, options: P4Options = {}): Promise<void> {
    if (!filePath) {
    ...
    try {
        ...
        // Check stderr for potential warnings even if command succeeds
        if (result.stderr) {
            context.outputChannel.appendLine(`Warning/Info during \`p4 edit ${filePath}\`: ${result.stderr}`);
            // Check for common non-error messages
            if (result.stderr.includes('already opened for edit') ||
                result.stderr.includes('can\'t edit exclusive file')) {
                // These might not be fatal errors depending on context
                context.outputChannel.appendLine('File was already opened or is exclusive.');
                // Decide if we should return or throw based on requirements
                return;
            }
        }
        context.outputChannel.appendLine(`\`p4 edit ${filePath}\` executed successfully.`);

    } catch (error: any) {
        // execute() already logs the error
        context.outputChannel.appendLine(`Error executing \`p4 edit ${filePath}\`: ${error.message}`);
        // Rethrow the error to indicate failure
        throw error;
    }
}
 ...
 * @returns A promise that resolves on success, rejects on failure.
 */
export async function p4delete(context: P4CommandContext, filePath: string, options: P4Options = {}): Promise<void> {
    if (!filePath) {
    ...
    try {
        ...
        if (result.stderr) {
            context.outputChannel.appendLine(`Warning/Info during \`p4 delete ${filePath}\`: ${result.stderr}`);
        ...
        context.outputChannel.appendLine(`\`p4 delete ${filePath}\` executed successfully.`);

    } catch (error: any) {
        context.outputChannel.appendLine(`Error executing \`p4 delete ${filePath}\`: ${error.message}`);
 ...
 * @returns A promise that resolves on success, rejects on failure.
 */
export async function p4revert(context: P4CommandContext, filePath: string, options: P4Options = {}): Promise<void> {
    if (!filePath) {
    ...
    try {
        ...
        if (result.stderr) {
            context.outputChannel.appendLine(`Info during \`p4 revert ${filePath}\`: ${result.stderr}`);
            ...
            if (result.stderr.includes('not opened on this client')) {
                context.outputChannel.appendLine('File was not opened, revert had no effect.');
                return; // Treat as success
            }
        }
         // Stdout often confirms the revert, e.g., "//depot/path#rev - was edit, reverted"
        if (result.stdout) {
             context.outputChannel.appendLine(`\`p4 revert ${filePath}\` output: ${result.stdout.trim()}`);
        }
        context.outputChannel.appendLine(`\`p4 revert ${filePath}\` executed successfully (or file was not open).`);

    } catch (error: any) {
        context.outputChannel.appendLine(`Error executing \`p4 revert ${filePath}\`: ${error.message}`);
 ...
 * @returns A promise that resolves on success, rejects on failure.
 */
export async function p4sync(context: P4CommandContext, filePaths: string[] = [], options: P4Options = {}): Promise<void> {
    ...
    try {
        ...
        if (result.stdout) {
             context.outputChannel.appendLine(`\`p4 sync ${target}\` stdout:\n${result.stdout.trim()}`);
        }
        if (result.stderr) {
            // Stderr might contain 'up-to-date' messages which aren't errors
            context.outputChannel.appendLine(`\`p4 sync ${target}\` stderr:\n${result.stderr.trim()}`);
        ...
        context.outputChannel.appendLine(`Error executing \`p4 sync ${target}\`: ${error.message}`);
 ...
 * @returns A promise that resolves on success, rejects on failure.
 */
export async function p4move(context: P4CommandContext, fromFilePath: string, toFilePath: string, options: P4Options = {}): Promise<void> {
    if (!fromFilePath || !toFilePath) {
        throw new Error('Both source and target file paths must be provided for p4 move.');
    }
    context.outputChannel.appendLine(`Executing \`p4 move ${fromFilePath} ${toFilePath}\`...`);
    try {
        // Specify from and to paths as arguments
        // No tagged output available/needed for move
        const result = await context.execute('move', [fromFilePath, toFilePath], options, false);

        // Check stderr for info/warnings
        if (result.stderr) {
            context.outputChannel.appendLine(`Warning/Info during \`p4 move\`: ${result.stderr}`);
        ...
        // Stdout usually confirms the move, e.g., "//depot/toFile#1 - moved from //depot/fromFile#1"
         if (result.stdout) {
             context.outputChannel.appendLine(`\`p4 move\` output: ${result.stdout.trim()}`);
         }
        context.outputChannel.appendLine(`\`p4 move ${fromFilePath} -> ${toFilePath}\` executed successfully.`);

    } catch (error: any) {
        context.outputChannel.appendLine(`Error executing \`p4 move ${fromFilePath} ${toFilePath}\`: ${error.message}`);
 ...
 */
export async function p4diff2(context: P4CommandContext, file1: string, file2: string, options: P4Options = {}, diffFlags: string[] = []): Promise<string> {
    if (!file1 || !file2) {
        ...
        if (result.stderr && !result.stderr.includes('no differences')) {
            ...
            // Check for common errors like "file not found"
             if (result.stderr.includes('no such file') || result.stderr.includes('not in client view')) {
                 throw new Error(`Diff failed: One or both files not found or not mapped (${file1}, ${file2}).`);
             }
        }
 ...
 */
export async function p4resolve(context: P4CommandContext, resolveFlags: string[], filePaths: string[] = [], options: P4Options = {}): Promise<string> {
    if (!resolveFlags || resolveFlags.length === 0) {
        throw new Error('Resolve flags (e.g., -n, -am, -ay) must be provided for p4 resolve.');
    }
    ...
    try {
        ...
        context.outputChannel.appendLine(`\`${commandDesc}\` completed. Check output for details.`);
        return output;
