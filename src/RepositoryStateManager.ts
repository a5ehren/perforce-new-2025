...
export class RepositoryStateManager implements vscode.Disposable {
  ...
  public async updateState(resourceUri?: vscode.Uri): Promise<void> {
    if (this.isUpdating) {
      this.outputChannel.appendLine(
        "Skipping updateState: Already in progress.",
      );
      return;
    }
    ...
    try {
      ...
      const openedFiles = this.processP4Result(
        openedResult,
        this.parseOpenedOutput.bind(this),
        "p4 opened",
      );
      ...
      const statusFiles = this.processP4Result(
        statusResult,
        this.parseStatusOutput.bind(this),
        "p4 status",
      );
      ...
      statusFiles.forEach((statusFile) => {
        ...
        if (existingFile) {
          ...
          this.outputChannel.appendLine(
            `Merging status info into existing file: ${key} (Status: ${statusFile.status})`,
          );
          ...
          existingFile.depotPath =
            existingFile.depotPath || statusFile.depotPath; // Fill if missing
          existingFile.clientPath =
            existingFile.clientPath || statusFile.clientPath; // Fill if missing
          ...
          existingFile.headRevision =
            existingFile.headRevision ?? statusFile.headRevision;
          existingFile.haveRevision =
            existingFile.haveRevision ?? statusFile.haveRevision;
          existingFile.type = existingFile.type ?? statusFile.type;
        } else {
          // Add new file found only by status
          this.outputChannel.appendLine(
            `Adding new file from status: ${key} (Status: ${statusFile.status})`,
          );
      ...
      this.outputChannel.appendLine(
        "Executing `p4 changes -s pending -l -G`...",
      );
      const changesArgs = ["-s", "pending", "-l"];
      if (this.p4Options.P4USER) {
        changesArgs.push("-u", this.p4Options.P4USER);
      }
      if (this.p4Options.P4CLIENT) {
        changesArgs.push("-c", this.p4Options.P4CLIENT);
      }
      const changesResult = await this.perforceService.execute(
        "changes",
        changesArgs,
        this.p4Options,
        true,
      ...
      const pendingChanges = this.processP4Result(
        changesResult,
        this.parseChangesOutput.bind(this),
        "p4 changes",
      );
          ...
          const promise = this.perforceService
            ...
            .then((result: P4Result) =>
              this.processP4Result(
                result,
                (data) => this.parseDescribeOutput(data, change.id),
                `p4 describe ${change.id}`,
              ),
            )
            .catch((err: Error) => {
              this.outputChannel.appendLine(
                `Failed to fetch/parse describe for ${change.id}: ${err.message}`,
              );
              return [];
            });
      ...
      allShelvedFiles.forEach((shelvedFile) => {
        ...
        if (existingFile) {
          ...
          this.outputChannel.appendLine(
            `Merging shelved status into existing file: ${key} (CL ${shelvedFile.changelist})`,
          );
          ...
          this.outputChannel.appendLine(
            `Adding new shelved file entry: ${key} (CL ${shelvedFile.changelist})`,
          );
      ...
      this.outputChannel.appendLine(
        `Re-associating ${this.files.size} resolved files with changelists...`,
      );
      this.files.forEach((file) => {
        ...
        if (!change) {
          ...
          this.outputChannel.appendLine(
            `Creating placeholder for changelist ${changeId} based on resolved file.`,
          );
          change = {
            id: changeId,
        ...
        if (
          !change.files.some((f) => f.uri.toString() === file.uri.toString())
        ) {
      ...
      previousChangeKeys.forEach((key) => {
        if (!currentChangeKeys.has(key)) {
          this.outputChannel.appendLine(
            `Pruning changelist no longer pending: ${key}`,
          );
      ...
      // --- Finalize ---
      this.outputChannel.appendLine(
        `Finished updating Perforce repository state. Files: ${this.files.size}, Changes: ${this.changelists.size}`,
      );
      this._onDidChange.fire();
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      this.outputChannel.appendLine(
        `Error updating repository state: ${errorMsg}`,
      );
  ...
  private processP4Result<T>(
    result: P4Result,
    parseMethod: (data: any[]) => T[],
    commandName: string,
  ): T[] {
    if (result.parsedOutput) {
      try {
        ...
        this.outputChannel.appendLine(
          `Error during parsing stage for \`${commandName}\`: ${parseError.message}`,
        );
        console.error(
          `Parsing error for ${commandName}`,
          parseError,
          result.parsedOutput,
        );
      }
    } else if (result.stdout && !result.stderr) {
      // Only warn if stdout exists AND stderr is empty (might be marshal error in stderr)
      this.outputChannel.appendLine(
        `Warning: Could not parse tagged output for \`${commandName}\` (parsedOutput field empty). Raw stdout present but parsing may have failed in PerforceService.`,
      );
      // Potentially attempt non-G parsing here if implemented
    } else if (!result.stdout && !result.stderr) {
      this.outputChannel.appendLine(
        `No output received from \`${commandName}\`.`,
      );
    } else {
      // If stderr is present, PerforceService likely already logged the command failure or marshal parse error
      this.outputChannel.appendLine(
        `Command \`${commandName}\` likely failed or parsing error occurred (stderr was present). Check previous logs.`,
      );
    }
  ...
  private parseOpenedOutput(p4Data: any[]): P4File[] {
    this.outputChannel.appendLine(
      `Parsing tagged \`p4 opened\` output (${p4Data?.length ?? 0} items)...`,
    );
    const files: P4File[] = [];
    if (!Array.isArray(p4Data)) {
      this.outputChannel.appendLine(
        `Error: Expected array from tagged output for 'opened', got ${typeof p4Data}`,
      );
    ...
    for (const record of p4Data) {
      if (
        ...
        this.outputChannel.appendLine(
          `Warning: Skipping invalid/incomplete opened record: ${JSON.stringify(record)}`,
        );
        continue;
      }
      ...
      if (!clientPath) {
        this.outputChannel.appendLine(
          `Warning: Skipping opened record missing clientFile: ${record.depotFile}`,
        );
        continue;
      }
    ...
    this.outputChannel.appendLine(
      `Parsed ${files.length} files from \`p4 opened\`.`,
    );
    return files;
  }
  ...
  private parseChangesOutput(p4Data: any[]): P4Changelist[] {
    this.outputChannel.appendLine(
      `Parsing tagged \`p4 changes\` output (${p4Data?.length ?? 0} items)...`,
    );
    const changes: P4Changelist[] = [];
    if (!Array.isArray(p4Data)) {
      this.outputChannel.appendLine(
        `Error: Expected array from tagged output for 'changes', got ${typeof p4Data}`,
      );
    ...
    for (const record of p4Data) {
      if (
        ...
        this.outputChannel.appendLine(
          `Warning: Skipping invalid/incomplete changes record: ${JSON.stringify(record)}`,
        );
        continue;
      }
      ...
      if (record.time) {
        try {
          ...
          this.outputChannel.appendLine(
            `Warning: Could not parse timestamp "${record.time}" for changelist ${record.change}: ${e.message}`,
          );
        }
      }
    ...
    this.outputChannel.appendLine(
      `Parsed ${changes.length} changelists from \`p4 changes\`.`,
    );
    return changes;
  }
  ...
  private parseDescribeOutput(p4Data: any[], changelistId: string): P4File[] {
    this.outputChannel.appendLine(
      `Parsing tagged \`p4 describe -s -S ${changelistId}\` output...`,
    );
    const files: P4File[] = [];
    // p4 describe -G output is typically a list containing one dictionary for the changelist description
    if (
      !Array.isArray(p4Data) ||
      p4Data.length === 0 ||
      typeof p4Data[0] !== "object" ||
      p4Data[0] === null
    ) {
      this.outputChannel.appendLine(
        `Error: Expected array with at least one object for 'describe ${changelistId}', got ${JSON.stringify(p4Data).substring(0, 200)}`,
      );
    ...
    while (record[`depotFile${i}`]) {
      ...
      if (!depotPath) {
        this.outputChannel.appendLine(
          `Warning: Skipping invalid/incomplete describe record index ${i} for CL ${changelistId} (missing depotPath)`,
        );
        i++;
        continue;
      }
    ...
    this.outputChannel.appendLine(
      `Parsed ${files.length} shelved files from \`p4 describe ${changelistId}\`.`,
    );
    return files;
  }
  ...
  private parseStatusOutput(p4Data: any[]): P4File[] {
    this.outputChannel.appendLine(
      `Parsing tagged \`p4 status\` output (${p4Data?.length ?? 0} items)...`,
    );
    const files: P4File[] = [];
    if (!Array.isArray(p4Data)) {
      this.outputChannel.appendLine(
        `Error: Expected array from tagged output for 'status', got ${typeof p4Data}`,
      );
    ...
    for (const record of p4Data) {
      ...
      if (
        typeof record !== "object" ||
        record === null ||
        (!record.clientFile && !record.depotFile)
      ) {
        this.outputChannel.appendLine(
          `Warning: Skipping invalid/incomplete status record: ${JSON.stringify(record)}`,
        );
        continue;
      }
      ...
      if (!pathForUri) {
        this.outputChannel.appendLine(
          `Warning: Skipping status record missing both clientFile and depotFile: ${JSON.stringify(record)}`,
        );
        continue;
      }
    ...
    this.outputChannel.appendLine(
      `Parsed ${files.length} files from \`p4 status\`.`,
    );
    return files;
  }
  ...
  private updateFiles(newFiles: P4File[]): Set<string> {
    this.outputChannel.appendLine(
      `Updating files map with ${newFiles.length} opened files.`,
    );
    ...
    newFiles.forEach((file) => {
      ...
      if (!change && changeId !== "default") {
        ...
        this.outputChannel.appendLine(
          `Creating placeholder for changelist ${changeId} based on opened file.`,
        );
        change = {
          id: changeId,
      ...
      if (change) {
        ...
        this.outputChannel.appendLine(
          `Warning: Could not find changelist ${changeId} to add file ${key}`,
        );
      }
    });
    this.outputChannel.appendLine(
      `File map size after update: ${this.files.size}`,
    );
  ...
  private updateChangelists(newChanges: P4Changelist[]): Set<string> {
    this.outputChannel.appendLine(
      `Updating changelist map with ${newChanges.length} pending changelists.`,
    );
    ...
    this.outputChannel.appendLine(
      `Changelist map size after update: ${this.changelists.size}`,
    );
  ...
  private updateShelvedFiles(shelvedFiles: P4File[]): void {
    this.outputChannel.appendLine(
      `Updating state with ${shelvedFiles.length} shelved files.`,
    );
    ...
    shelvedFiles.forEach((shelvedFile) => {
      ...
      if (existingFile) {
        ...
        if (change) {
          ...
          this.outputChannel.appendLine(
            `Warning: Could not find parent changelist ${shelvedFile.changelist} for shelved file ${key}`,
          );
        }
      }
    });
    this.outputChannel.appendLine(
      `File map size after shelved update: ${this.files.size}`,
    );
  }
  ...
  private updateStatusFiles(statusFiles: P4File[]): void {
    this.outputChannel.appendLine(
      `Updating state with ${statusFiles.length} files from p4 status.`,
    );
    ...
    statusFiles.forEach((statusFile) => {
      ...
      if (existingFile) {
        ...
        this.outputChannel.appendLine(
          `Updating existing file ${key} with status: ${statusFile.status}`,
        );
        ...
        existingFile.diffStatus =
          existingFile.diffStatus ?? statusFile.diffStatus; // Keep existing diffStatus if already set
        // Potentially update action if status implies something different?
        // e.g., if opened for edit but status says 'needsDelete'
        if (
          existingFile.action !== statusFile.action &&
          statusFile.action !== "unknown"
        ) {
          this.outputChannel.appendLine(
            ` Action mismatch for ${key}: opened='${existingFile.action}', status='${statusFile.action}'. Keeping opened action for now.`,
          );
        ...
        this.outputChannel.appendLine(
          `Adding new file ${key} from status: ${statusFile.status}`,
        );
        this.files.set(key, statusFile);
        ...
        if (change) {
          ...
          this.outputChannel.appendLine(
            `Warning: Default changelist not found when adding file from status: ${key}`,
          );
        }
      }
    });
    this.outputChannel.appendLine(
      `File map size after status update: ${this.files.size}`,
    );
  }
  ...
  private async resolveFileUris(): Promise<void> {
    ...
    this.outputChannel.appendLine(
      `Found ${uniqueClientPaths.length} unique client paths to query.`,
    );
    ...
    const resolutionPromises = uniqueClientPaths.map(async (clientPath) => {
      try {
        ...
        const localPath = await this.perforceService.getLocalPath(
          clientPath,
          this.p4Options,
        );
        ...
        this.outputChannel.appendLine(
          `Error resolving local path for "${clientPath}": ${error.message}`,
        );
    ...
    this.outputChannel.appendLine(
      `Finished querying local paths. Updating file URIs...`,
    );
    ...
    for (const file of filesToResolve) {
      ...
      if (!file.clientPath || file.clientPath.length === 0) {
        this.outputChannel.appendLine(
          `Skipping file with empty client path: ${key}`,
        );
      ...
      if (localPath) {
        try {
          ...
          if (resolvedFileKeys.has(newKey) && key !== newKey) {
            this.outputChannel.appendLine(
              `Warning: Multiple client paths resolved to the same local URI "${newKey}". Original placeholder: ${key}, ClientPath: ${file.clientPath}. Keeping first occurrence.`,
            );
          ...
          this.outputChannel.appendLine(
            ` Resolved "${file.clientPath}" -> "${localPath}"`,
          );
          ...
          this.outputChannel.appendLine(
            `Error creating file URI for resolved path "${localPath}" (from clientPath "${file.clientPath}"): ${uriError.message}. Removing file.`,
          );
          unresolvedFiles.push(key); // Mark for removal
        }
      } else {
        this.outputChannel.appendLine(
          ` Failed to resolve local path for client path "${file.clientPath}". Removing file: ${key}`,
        );
        unresolvedFiles.push(key); // Mark for removal
      }
    }
    ...
    this.outputChannel.appendLine(
      `Finished resolving URIs. Final file count: ${this.files.size}.`,
    );
  }
